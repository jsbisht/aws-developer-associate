...
When replication happens within the same AWS account, both buckets are owned by same AWS account and they both trust the same AWS account. They both trust IAM and IAM role.

Whereas when replication between different account happens, IAM role configured for replication isn't trusted by the destination account.
